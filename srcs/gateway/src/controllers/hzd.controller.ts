...
const HZD_SERVICE_URL = 'http://hazardous-service:3017';

export function registerHazardousRoutes(app: FastifyInstance) {
  // Route health spécifique
  app.get('/health', async (request: FastifyRequest, reply: FastifyReply) => {
    logger.logHealth({ serviceName: 'auth-service' }, 'service_check');
    const res = await proxyRequest(app, request, reply, `${HZD_SERVICE_URL}/health`);
    return res;
  });
  ...
  app.all('/*', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const cleanPath = rawPath.replace(/^api\/hazardous\//, '');
    const url = `${HZD_SERVICE_URL}/${cleanPath}`;
